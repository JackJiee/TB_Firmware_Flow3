...
namespace TB_Firmware_Flow3.Manager
{
    class XboxOneGip
    {
        ...
        private bool[] connectType = new bool[16];
        private uint[] connectVersion = new uint[16];
       ...
       public bool Connected()
            ...
            lock (myLock)
            {
                ...
                if (!deviceInList && Vid == 0x10f5 && (Pid == 0x7007 || Pid == 0x7008 || Pid == 0x7005))
                {
                   ...
                   // firwmwareVerionos = device.FirmwareVersionInfo.Build;
                    ...
                    if(Pid==0x7008)
                    {
                        deviceHandle[(int)(ControllerType.ReconWhite)] = device;
                        upgradeFinish = false;
                        connectType[(int)ControllerType.ReconWhite] = true;
                        connectVersion[(int)ControllerType.ReconWhite] = device.HardwareVersionInfo.Build;
                    }
                    if(Pid==0x7005)
                    {
                        deviceHandle[(int)(ControllerType.ReconBlack)] = device;
                        upgradeFinish = false;
                        connectType[(int)ControllerType.ReconBlack] = true;
                        connectVersion[(int)ControllerType.ReconBlack] = device.HardwareVersionInfo.Build;
                    }
               //     connectStatic = true;
                    IsConnected(true);
                    if (Pid == 0x7007)
                    {
                        deviceHandle[(int)(ControllerType.BootLoaderMode)] = device;
                        UpgradeMode = true;
                        if (UpgradeFlag)
                        {
            ...
            lock (myLock)
            {
                ...
                if (deviceInList && Vid == 0x10f5 && (Pid == 0x7007 || Pid == 0x7008 || Pid == 0x7005))
                {
                   ...
                   // connectStatic = false;
                    ...
                    {
                        deviceHandle[(int)(ControllerType.ReconWhite)] = device;
                        connectType[(int)ControllerType.ReconWhite] = false;
                    }
                    if (Pid == 0x7005)
                    {
                        deviceHandle[(int)(ControllerType.ReconBlack)] = device;
                        connectType[(int)ControllerType.ReconBlack] = false;
                    }
                    if (Pid == 0x7007)//
                    {
                        deviceHandle[(int)(ControllerType.BootLoaderMode)] = device;
        ...
        public void  SetUpdateData(int type,byte[]data)
        {
            dataBuffer[type] = data;
        }
        ...
        public bool StateUpgrade(int type=0)
        {
            if (device_xbox == null)
                return false;
            if (deviceHandle[(int)(ControllerType.BootLoaderMode)]!=null)
                updcon.IniDeviceXbox(deviceHandle[(int)(ControllerType.BootLoaderMode)]);
            else 
                updcon.IniDeviceXbox(deviceHandle[type]);
            if(type==(int)ControllerType.ReconWhite)
            {
                writeBuffer =  dataBuffer[(int)ControllerType.ReconWhite];
            } 
            else if (type == (int)ControllerType.ReconBlack)
            {
                writeBuffer = dataBuffer[(int)ControllerType.ReconBlack];
            } 
            else if (type == (int)ControllerType.VelocityOneFlight)
            {
                writeBuffer = dataBuffer[(int)ControllerType.VelocityOneFlight];
            }
                UpgradeFlag = true;
            error = false;
            upgradeFinish = false;
        ...
        public async System.Threading.Tasks.Task ReadWhiteFileAsync()//读取文件
        {
            ...
            dataBuffer[(int)ControllerType.ReconWhite] = new byte[fileSize];
            for (uint i = 0; i < buffer.Length; i++)
            {
                dataBuffer[(int)ControllerType.ReconWhite][i] = buffer.GetByte(i);
            }
        ...
        public async System.Threading.Tasks.Task ReadBlackFileAsync()//读取文件
        {
            ...
            var buffer = await Windows.Storage.FileIO.ReadBufferAsync(sampleFile);
            fileSize = buffer.Length;
            dataBuffer[(int)ControllerType.ReconBlack] = new byte[fileSize];
            for (uint i = 0; i < buffer.Length; i++)
            {
                dataBuffer[(int)ControllerType.ReconBlack][i] = buffer.GetByte(i);
            }
            byte by = (byte)buffer.GetByte(1);
            Debug.Write("******" + buffer.GetByte(0).ToString() + buffer.GetByte(1).ToString() + "*U**" + buffer.Length.ToString() + "\n");
